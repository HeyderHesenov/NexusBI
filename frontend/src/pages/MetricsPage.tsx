...
export function MetricsPage() {
  ...
  const sourceName = (id: string | null) =>
    id ? sources.find((s) => s.id === id)?.name ?? t('metricsPage.source') : t('metricsPage.demoGlobal')

  return (
    <div className="w-full">
      <header className="mb-6 flex items-end justify-between gap-4">
        <div>
          <p className="eyebrow">{t('metricsPage.semanticLayer')}</p>
          <h1 className="mt-1 font-display text-3xl font-bold tracking-tight text-ink">{t('metricsPage.title')}</h1>
        ...
        >
          <Plus size={15} /> {t('metricsPage.addMetric')}
        </button>
      </header>
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {m.verified && (
                      <span
                        title={t('metricsPage.verifiedBy', { by: m.verified_by ?? '—' })}
                      ...
                      >
                        <BadgeCheck size={11} /> {t('metricsPage.verified')}
                      </span>
                    )}
                    {m.expression && (
                  ...
                  {m.synonyms && (
                    <p className="mt-1 text-xs text-ink-faint">{t('metricsPage.synonymsLabel')}: {m.synonyms}</p>
                  )}
                ...
                <div className="flex shrink-0 items-center gap-1">
                  <button
                    ...
                    title={m.verified ? t('metricsPage.unverify') : t('metricsPage.verify')}
...
function AddMetricModal({
  ...
  return (
    <ModalShell
      open={open}
      onClose={onClose}
      title={t('metricsPage.addMetric')}
      subtitle={t('metricsPage.modalSubtitle')}
      footer={
