...
const note = (id: string, read = false, title = 'insight') => ({ id, read, title }) as never
...
describe('notificationStore.load', () => {
  ...
  it('does not toast brief (🌅) notifications', async () => {
    list.mockResolvedValue([note('x', false)] as never)
    await useNotificationStore.getState().load()
    list.mockResolvedValue([note('x', false), note('brief', false, '🌅 Səhər brifi')] as never)
    await useNotificationStore.getState().load()
